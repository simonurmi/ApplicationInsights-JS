...
export function createChannelControllerPlugin(channelQueue: _IInternalChannels[], core: IAppInsightsCore): IChannelController {
    ...
    function _processChannelQueue<T extends IBaseProcessingContext>(itemCtx: T, processFn: (chainCtx: T) => void, onComplete: () => void) {
        let waiting = channelQueue ? (channelQueue.length + 1) : 1;
    ...
    function _doUpdate(updateCtx: IProcessTelemetryUpdateContext, updateState: ITelemetryUpdateState) {
        ...
        _processChannelQueue(updateCtx, (chainCtx: IProcessTelemetryUpdateContext) => {
    ...
    function _doTeardown(unloadCtx: IProcessTelemetryUnloadContext, unloadState: ITelemetryUnloadState) {
        ...
        _processChannelQueue(unloadCtx, (chainCtx: IProcessTelemetryUnloadContext) => {
    ...
    let channelController: IChannelController = {
        ...
        processTelemetry: (item: ITelemetryItem, itemCtx: IProcessTelemetryContext) => {
            _processChannelQueue(itemCtx || _getTelCtx(), (chainCtx: IProcessTelemetryContext) => {
        ...
        update: _doUpdate,
        pause: () => {
            _processChannelQueue(_getTelCtx(), (chainCtx: IProcessTelemetryContext) => {
