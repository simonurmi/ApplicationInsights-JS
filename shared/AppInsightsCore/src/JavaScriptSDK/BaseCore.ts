...
import { arrForEach, isNullOrUndefined, toISOString, getSetValue, setValue, throwError, isNotTruthy, isFunction, objFreeze, proxyFunctionAs, proxyFunctions, objExtend } from "./HelperFuncs";
...
import { strDisabled, strTeardown } from "./InternalConstants";
...
export class BaseCore implements IAppInsightsCore {
    ...
    constructor() {
        ...
        dynamicProto(BaseCore, this, (_self) => {
            ...
            _self.updateConfig = <T extends IConfiguration = IConfiguration>(newConfig: T, mergeExisting: boolean = true) => {
                if (mergeExisting) {
                    newConfig = objExtend({}, _self.config, newConfig);
                }
                ...
                _self.config = newConfig;

                _initDebugListener(newConfig);
                _initPerfManager(newConfig);
                if (_self.logger && _self.logger.update) {
                    _self.logger.update(updateState);
                }

                if (_cookieManager && _cookieManager.update) {
                    _cookieManager.update(updateState)
                }
            ...
            function _getPlugin<T extends IPlugin = IPlugin>(pluginIdentifier: string): ILoadedPlugin<T> {
                ...
                arrForEach(_self._extensions, (ext: any) => {
                    if (ext.identifier === pluginIdentifier && ext !== _channelControl) {
                        thePlugin = ext;
                        return -1;
                    }
                });
