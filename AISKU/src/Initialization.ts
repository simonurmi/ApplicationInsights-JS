...
import {
    ...
    createUniqueNamespace, ITelemetryPlugin, IPlugin, UnloadHandler, removePageUnloadEventListener, removePageHideEventListener, ITelemetryInitializerHandler
...
const _ignoreUpdateSnippetProperties = [
    "snippet", "dependencies", "properties", "_snippetVersion", "appInsightsNew"
];
...
export const Telemetry = {
    ...
    //AIData: createClassFromInterface<AIData>(),
    //AIBase: createClassFromInterface<AIBase>(),
    Envelope,
    Event,
    Exception,
    Metric,
    PageView,
    // PageViewData : createClassFromInterface<IPageViewData>(),
...
export class Initialization implements IApplicationInsights {
        ...
        dynamicProto(Initialization, this, (_self) => {
            ...
            _self.addHousekeepingBeforeUnload = (appInsightsInstance: IApplicationInsights): void => {
                ...
                if (hasWindow() || hasDocument()) {
                    ...
                    let eventNamespace = [ _evtNamespace, _core.evtNamespace && _core.evtNamespace()];
                    let hasAddUnloadCb = !!_core[strAddUnloadCb];

                    if (!appInsightsInstance.appInsights.config.disableFlushOnBeforeUnload) {
                        // Hook the unload event for the document, window and body to ensure that the client events are flushed to the server
                        // As just hooking the window does not always fire (on chrome) for page navigation's.
                        if (addPageUnloadEventListener(performHousekeeping, excludePageUnloadEvents, eventNamespace)) {
                            added = true;
                            hasAddUnloadCb && _core[strAddUnloadCb](() => {
                                removePageUnloadEventListener(performHousekeeping, eventNamespace);
                            });
                        }
        
                        // We also need to hook the pagehide and visibilitychange events as not all versions of Safari support load/unload events.
                        if (addPageHideEventListener(performHousekeeping, excludePageUnloadEvents, eventNamespace)) {
                            added = true;
                            hasAddUnloadCb && _core[strAddUnloadCb](() => {
                                removePageHideEventListener(performHousekeeping, eventNamespace);
                            });
                        }
            ...
            _self.unload = (isAsync?: boolean): void => {
                _self.onunloadFlush(isAsync);
                _core.unload && _core.unload(isAsync);
            };
     ...
     * @param mergeExisting - Should the new configuration merge with the existing or just replace it. Default is to merge.
     */
    public updateConfig<T extends IConfiguration = IConfiguration>(newConfig: T, mergeExisting?: boolean): void {
        // @DynamicProtoStub -- DO NOT add any code as this will be removed during packaging
    }
